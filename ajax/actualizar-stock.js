$(document).ready(function () {
  ...
  $(document).prop("title", "Actualizar Stock - Zenith Group");
});
...
$("#FRM_INSERT_MOV").submit(function (e) {
  ...
  if ($('input[name="mov_cantidad"]').val() == 0) {
    toastr.error(
      "Especifique un valor movimiento de stock diferente de 0.",
      "Stock no válido"
    );
    return;
  }
  ...
  if ($("#rbtnIngreso").is(":checked")) {
    if ($('select[name="mov_prov"]').val() == 0) {
      toastr.error(
        "Seleccione un proveedor para asociarlo a este ingreso.",
    ...
    if ($('input[name="mov_cantidad"]').val() < 0) {
      toastr.error(
        "El stock de ingreso no puede ser negativo.",
        "Stock no válido"
      );
      return;
    }
    ...
    success: function (data) {
      if (data == "ERROR") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "No se pudieron guardar datos del movimiento de stock"
        );
        Swal.close();
