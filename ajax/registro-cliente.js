...
$(document).ready(function () {
  ...
  $(document).prop("title", "Clientes - Zenith Group");
});
    ...
    success: function (data) {
      if (data == "ERROR") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "No se pudo guardar datos del cliente"
        );
        Swal.close();
      } else if (data == "EXISTE") {
        $.Notification.notify(
...
$.post(
  "../../modules/ubigeo/consultar-ubigeo.php",
  { MODE_UBIGEO: "DEPARTMENTS" },
  function (data) {
...
$('select[name="cliente_departamento"]').on("change", function (e) {
  ...
  $.post(
    "../../modules/ubigeo/consultar-ubigeo.php",
    { MODE_UBIGEO: "PROVINCES", ID_DEPART: ID_DEPART },
    function (data) {
...
$('select[name="cliente_provincia"]').on("change", function (e) {
  ...
  $.post(
    "../../modules/ubigeo/consultar-ubigeo.php",
    { MODE_UBIGEO: "DISTRICTS", ID_PROV: ID_PROV },
    function (data) {
    ...
    function (data) {
      ...
      $.post(
        ...
        function (data) {
          ...
          $('select[name="cliente_departamento"]').val(
            data_json[0]["DEPARTAMENTO"]
          );
        ...
        $.post(
          "../../modules/ubigeo/consultar-ubigeo.php",
          { MODE_UBIGEO: "PROVINCES", ID_DEPART: data_json[0]["DEPARTAMENTO"] },
          function (data) {
            $('select[name="cliente_provincia"]').empty();
            $('select[name="cliente_provincia"]').select2({
              data: JSON.parse(data),
            });
            $('select[name="cliente_provincia"]').val(
              data_json[0]["PROVINCIA"]
            );
          ...
          $.post(
            "../../modules/ubigeo/consultar-ubigeo.php",
            { MODE_UBIGEO: "DISTRICTS", ID_PROV: data_json[0]["PROVINCIA"] },
            function (data) {
              $('select[name="cliente_distrito"]').empty();
              $('select[name="cliente_distrito"]').select2({
                data: JSON.parse(data),
              });
              $('select[name="cliente_distrito"]').val(
                data_json[0]["DISTRITO"]
              );
...
$("#btn-delete-client").click(function () {
  ...
  if (id_val != "" && id_val != null) {
    Swal.fire({
      ...
      if (result.value) {
        $.post(
          "../../modules/clientes/eliminar-cliente.php",
          { cliente_id: id_val },
          function (data) {
            if (data == true) {
