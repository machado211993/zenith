...
$(document).ready(function () {
  ...
  $(document).prop("title", "Orden de Compra - Zenith Group");
});
...
$.post(
  "../../modules/ordenes/listar-ordenes.php",
  { orden_tipo: "COMPRA" },
  function (data) {
...
$("#table-ord-compras").on("dblclick", "tr", function () {
  ...
  tbl_prodordcompra.rows(tbl_prodordcompra.row(this)).remove().draw();
  ...
  if (DATA_ID != "" && DATA_ID != null) {
    ...
    $.post(
      ...
      function (data) {
        ...
        $('input[name="orden_igv"]').val(
          (data_json[0]["TOTIGV"] * 1).toFixed(2)
        );
        $('input[name="orden_totcompra"]').val(
          (data_json[0]["TOTCOMP"] * 1).toFixed(2)
        );
        $('input[name="orden_totneto"]').val(
          (data_json[0]["TOTNET"] * 1).toFixed(2)
        );
    ...
    success: function (data) {
      if (data == "ERROR") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "No se pudo guardar datos de orden de compra"
        );
        Swal.close();
