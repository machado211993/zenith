...
$(document).ready(function () {
  ...
  $(document).prop("title", "Usuarios - Zenith Group");
});
...
$.post(
  "../../modules/empleados/consultar-empleado.php",
  { FILTER: "SELECT_LIST" },
  function (data) {
    ...
    success: function (data) {
      if (data == "ERROR") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "No se pudo guardar datos del usuario"
        );
        Swal.close();
      } else if (data == "EXISTE") {
        $.Notification.notify(
...
tabla_usuarios.on("click", "tr", function () {
  ...
  $.post(
    "../../modules/usuarios/consultar-usuario.php",
    { FILTER: id_row },
    function (data) {
      ...
      $("#pass-label-1 font").html(
        "Nueva Contraseña (dejar en blanco para dejar sin cambios)"
      );
...
$("#btn-delete-user").click(function () {
  ...
  if (id_val != "" && id_val != null) {
    Swal.fire({
      ...
      if (result.value) {
        $.post(
          "../../modules/usuarios/eliminar-usuario.php",
          { user_id: id_val },
          function (data) {
            if (data == true) {
