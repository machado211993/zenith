...
$(document).ready(function () {
  ...
  $("#m_nota_credito")
    .parent()
    .attr("class", "nav-item has-treeview menu-open");
  $(document).prop("title", "Registro de Nota de Crédito - Zenith Group");
});
...
$('input[name="facturacion_valcliente"]').autocomplete({
  source: function (request, response) {
    $.getJSON(
      "../../modules/clientes/obtener-clientes.php",
      { cotiz_nomcliente: $('input[name="facturacion_valcliente"]').val() },
      response
    );
  },
    ...
    $.post(
      ...
      function (data) {
        ...
        if (data_json.length > 0) {
          ...
          $(
            'select[name="facturacion_listadofact"] option:selected'
          ).removeAttr("selected");

          if ($docType == 1) {
            $(
              "select[name='facturacion_listadofact'] option[value=" +
                data_json[0]["REFERENCED_DOC_ID"] +
                "]:contains('F')"
          ...
          $('input[name="facturacion_valcliente"]').val(
            data_json[0]["CLIENTNAME"]
          );

          $('input[name="facturacion_cliruc"]').val(data_json[0]["CLIENTRUC"]);
          $('input[name="facturacion_clidirecc"]').val(
            data_json[0]["CLIENTADDR"]
          );
          $('input[name="facturacion_clirefer"]').val(
            data_json[0]["CLIENTREFER"]
          );

          $('select[name="facturacion_formpagotext"]').val(
            data_json[0]["PAY_DAYS"]
          );
          ...
          if ($('select[name="facturacion_formpagotext"]').val() == null) {
            ...
            $('input[name="facturacion_formpago"]').val(
              data_json[0]["PAY_DAYS"]
            );
          }

          $('input[name="facturacion_fecentrega"]').val(
            data_json[0]["DELIV_DATE"]
          );
          ...
          $('input[name="facturacion_porcdesc"]').val(
            data_json[0]["DESC_RATE"]
          );
          $('input[name="facturacion_cantdesc"]').val(data_json[0]["DESC_VAL"]);
          $('input[name="facturacion_opergrab"]').val(
            parseFloat(data_json[0]["TOTAL_SUB"]).toFixed(2)
          );
          $('input[name="facturacion_igv"]').val(
            parseFloat(data_json[0]["TOTAL_TAX"]).toFixed(2)
          );
          $('input[name="facturacion_total"]').val(
            parseFloat(data_json[0]["TOTAL_NET"]).toFixed(2)
          );
            ...
            function (data) {
              ...
              for (i = 0; i < detafact_prods.length; i++) {
                var precio = parseFloat(
                  detafact_prods[i]["PRECIOUNIT"]
                ).toFixed(2);
                var importe = parseFloat(detafact_prods[i]["IMPORTE"]).toFixed(
                  2
                );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data == "ERROR") {
        ...
        $.post(
          "../../modules/facturacion/listar-notas-credito.php",
          function (data) {
            $('select[name="facturas_listado"]').empty();
            $('select[name="facturas_listado"]').select2({
              data: JSON.parse(data),
...
$("#btn-pendiente-factura").click(function () {
  ...
  if (id_val != "" && id_val != null) {
    Swal.fire({
      title:
        "¿Está seguro de marcar como PENDIENTE DE PAGO esta nota de crédito?",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
...
function postCambioEstado() {
  ...
  $("#FRM_INSERT_FACTURA").find("input, textarea, select").val("");
...
function buscarCorrelativo() {
  ...
  $.post(
    ...
    { TIPO_DOC: "CREDIT_NOTE", SERIE: serieFactura },
    function (data) {
      if (data != "" && data != null) {
