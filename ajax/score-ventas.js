...
$(document).ready(function () {
  ...
  $(document).prop("title", "Score de Ventas - Zenith Group");
});
...
$(document).ready(function () {
  ...
  $("#btn-filtrar-reporte").click(function () {
    ...
    if (
      val_nomvend != "" ||
      (val_periodo == "MONTH" && val_mes != "" && val_year != "") ||
      (val_periodo == "YEAR" && val_year != "")
    ) {
      Swal.fire({
  ...
  $('input[name="reporte_vendedor"]').autocomplete({
    source: function (request, response) {
      $.getJSON(
        "../../modules/usuarios/obtener-usuarios.php",
        { NOM_VEND: $('input[name="reporte_vendedor"]').val() },
        response
      );
    },
  ...
  function get_data_report(nom_vend, period_val, period_mes, period_year) {
    $.post(
      "../../modules/reportes/cargar-reporte-ventas-x-vendedor.php",
      {
        VENDEDOR: nom_vend,
        PERIODO: period_val,
        MONTH: period_mes,
