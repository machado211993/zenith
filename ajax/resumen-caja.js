...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('jornada_selector').addEventListener('change', function() {
...
function cargarDatosJornada(jornadaId) {
    ...
    const tbody = document.getElementById("tabla_movimientos_caja").querySelector("tbody");
    tbody.innerHTML = '<tr><td colspan="5" class="text-center">Cargando movimientos...</td></tr>';

    fetch(`../../modules/caja/consultar-movimientos-caja.php?jornada_id=${jornadaId}`)
        .then(response => response.json())
        .then(data => {
            if (data.exito) {
                const resumen = data.data.resumen;
                document.getElementById("jornada_actual").textContent = data.data.jornada_id;
                document.getElementById("total_ingreso").textContent = `$ ${parseFloat(resumen.TotalIngreso || 0).toFixed(2)}`;
                document.getElementById("total_egreso").textContent = `$ ${parseFloat(resumen.TotalEgreso || 0).toFixed(2)}`;
                document.getElementById("total_caja").textContent = `$ ${parseFloat(resumen.TotalCaja || 0).toFixed(2)}`;
                
                actualizarTablaMovimientos(data.data.movimientos);
            } else {
                mostrarError(data.mensaje || 'Error al cargar los datos de la jornada.');
            }
        })
        .catch(error => {
            console.error(error);
...
function actualizarTablaMovimientos(movimientos) {
    ...
    if (movimientos.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center">No hay movimientos registrados en esta jornada.</td></tr>';
    } else {
        movimientos.forEach(mov => {
            ...
            if (mov.tipo === "INGRESO" || mov.tipo === "INICIO") {
