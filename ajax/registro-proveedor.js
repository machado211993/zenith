...
$(document).ready(function () {
  ...
  $(document).prop("title", "Proveedores - Zenith Group");
});
...
$(
  'select[name="proveedor_tipmoneda_1"], select[name="proveedor_tipmoneda_2"]'
).select2({
  data: [
    { id: "", text: "Seleccione" },
    ...
    success: function (data) {
      if (data == "ERROR") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "No se pudo guardar datos del proveedor"
        );
        Swal.close();
      } else if (data == "EXISTE") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "Proveedor ya existe en la base de datos"
        );
        Swal.close();
...
tabla_proveedores.on("click", "tr", function () {
  ...
  $.post(
    ...
    function (data) {
      ...
      $('input[name="proveedor_ctacorriente_1"]').val(
        data_json[0]["BANK1_ACCNUM"]
      );
      $('input[name="proveedor_titularcta_1"]').val(
        data_json[0]["BANK1_ACCHOL"]
      );
      ...
      $('input[name="proveedor_ctacorriente_2"]').val(
        data_json[0]["BANK2_ACCNUM"]
      );
      $('input[name="proveedor_titularcta_2"]').val(
        data_json[0]["BANK2_ACCHOL"]
      );
...
$("#btn-delete-proveedor").click(function () {
  ...
  if (id_val != "" && id_val != null) {
    Swal.fire({
      ...
      if (result.value) {
        $.post(
          "../../modules/proveedores/eliminar-proveedor.php",
          { proveedor_id: id_val },
          function (data) {
            if (data == true) {
