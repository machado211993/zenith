...
$(document).ready(function () {
  ...
  $(document).prop("title", "Editar Producto - Zenith Group");
});
...
$(document).ready(function () {
  ...
  $.post(
    ...
    function (data) {
      ...
      $('input[name="producto_estado"]').prop(
        "checked",
        data_json[0]["ESTADO"]
      );
      ...
      $('input[name="producto_proveedor_referencia"]').val(
        data_json[0]["PROVEEDOR_REF"]
      );
    ...
    success: function (data) {
      if (data == "ERROR") {
        $.Notification.notify(
          ...
          "bottom-right",
          "Error de guardado",
          "No se pudo guardar datos del producto"
        );
        Swal.close();
      } else if (data == "EXISTE") {
        $.Notification.notify(
...
$("#btn-delete-product").click(function () {
  ...
  if (id_val != "" && id_val != null) {
    Swal.fire({
      ...
      if (result.value) {
        $.post(
          "../../modules/productos/eliminar-producto.php",
          { producto_id: id_val },
          function (data) {
            if (data == true) {
              window.location.replace("../../views/productos/listado-producto");
