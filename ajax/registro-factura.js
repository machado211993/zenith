...
$(document).ready(function () {
  ...
  $(document).prop("title", "Registro de Factura - Zenith Group");
});
    ...
    $.post(
      ...
      function (data) {
        ...
        if (data_json.length > 0) {
          ...
          $('input[name="facturacion_nro"]').val(
            data_json[0]["CODIGO_CORRELATIVO"]
          );
        ...
        $.post(
          ...
          function (data) {
            ...
            for (i = 0; i < detacotiz_json.length; i++) {
              var precio = parseFloat(detacotiz_json[i]["PRECIOUNIT"]).toFixed(
                2
              );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (response == "ERROR") {
        ...
        $.post(
          "../../modules/facturacion/listar-facturas.php",
          function (data) {
            $('select[name="facturas_listado"]').empty();
            $('select[name="facturas_listado"]').select2({
              data: JSON.parse(data),
